from calsim_scenario_server.models import MetricModel
...
def test_columns():
    columns = [c.key for c in MetricModel.__table__.columns]
    missing = list()
...
def test_column_types():
    columns = {c.key: c.type.python_type for c in MetricModel.__table__.columns}
    bad_types = list()
    for c in EXPECTED_COLUMNS:
